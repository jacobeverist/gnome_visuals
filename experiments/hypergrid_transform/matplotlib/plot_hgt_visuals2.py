...
def run(filename_root="visual_test_frame_1", output_dir="out"):
    ...
    # fig, axes = plt.subplots(1, 2, num=1, figsize=(8, 8), gridspec_kw={"width_ratios": [0.6, 0.5]})
    fig, axes = plt.subplots(1, 2, num=1, figsize=(8, 8), constrained_layout=True)
    ...
    for point_index in range(num_points):
        ...
        # ic(indices)
        ic(indices[0])
        ic(indices[0].astype(int) // int(hgt.num_bins))
        ic(indices[0]//hgt.num_bins + 1)

        X_gnomes[indices] = indices[0]//hgt.num_bins + 1
        ic(X_gnomes)

        ax1 = axes[1]
        ax1.clear()
        draw_gnomes(ax1, X_gnomes, hgt.num_bins, num_grids=hgt.num_grids, colors=colors2, fontsize=10)
