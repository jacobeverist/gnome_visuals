...
from arrange_bins import compute_simple_bins
...
class EncoderCollapse(Scene):
    ...
    def add_placed_brick(self, unfolded_brick, folded_brick):
        """An iterative way of building a visualization of the encoding.
        ...
        :param brick_geom: dict with keys: box_height, box_width, box_x, box_y
            {'box_height': 0.7,
             'box_width': np.float64(0.19600000000000006),
             'box_x': np.float64(0.7353333333333333),
             'box_y': 11.15},
        :return:
        """

        box_height = unfolded_brick['box_height']
        box_width = unfolded_brick['box_width']
        box_x = unfolded_brick['box_x']
        box_y = unfolded_brick['box_y']

        # add new place cell to encoder
        # FIXME:  why do I create a separate place encoder here?  Is the subject encoder not good enough?
        # FIXME:  I think it's just to compute the boundaries and then use them to make bricks
        ...
        start_level_height = self.nl.get_top()[1] + box_y + box_height / 2.0

        # create whole block

        # GEOMETRY AND LOCATION OF BINS

        # get bin bounds based on closest region boundaries
        # sec_lower = boundaries[lower_index]
        # sec_upper = boundaries[upper_index]
        sec_lower = box_x
        sec_upper = box_x + box_width

        # convert to SCENE coordinates
        scene_bin_lower = self.nl.number_to_point(sec_lower)[0]
        scene_bin_upper = self.nl.number_to_point(sec_upper)[0]

        # whole block width, x-center, and center point in SCENE coordinates
        ...
        # box_height = folded_brick['box_height']
        # box_width = folded_brick['box_width']
        # box_x = folded_brick['box_x']
        folded_box_y = folded_brick['box_y']

        mid_level_height = self.nl.get_top()[1] + folded_box_y + box_height / 2.0
    ...
    def shift_brick(self, cell_i):
        ...
        # set the rectangle and text objects to be invisible
        ...
        diff_anims = [Succession(whole_brick_anim)]
...
class AnimateEncoding(EncoderCollapse):
    ...
    def construct(self):
        ...
        # create encoder
        # subject_encoder = TaperingWeightEncoder(n=13, w=3)
        subject_encoder = MultiEncoder()
        subject_encoders = []
        for k in [7, 11, 13]:
            subject_encoders.append(TaperingWeightEncoder(n=k, w=3))
        ...
        for enc in subject_encoders:
            ...
            folded_draw_y = draw_max_y

            num_bricks = len(unfolded_rect_geoms)
            ic(num_bricks)
            if len(folded_rect_geoms) != len(unfolded_rect_geoms):
                raise Exception("number of bricks in folded and unfolded do not match")
        ...
        # enable bin highlighting when scalar value is within bin range
        self.enable_active_bins()
        ...
        # creating brick shifting animations
        all_anims = []
        for cell_i in range(self.curr_id):
            diff_anims = self.shift_brick(cell_i)
            if len(diff_anims) > 0:
                all_anims += diff_anims
