...
class NumberSlider(VGroup):
    def __init__(self, x_range=(0, 1, 0.2), length=10, tracker=None, encoder=None, **kwargs):
        ...
        decimal_number.add_updater(
                lambda obj: obj.set_value(
                        ...
                        # ).move_to(background_rectangle.get_center())
        ...
        #
        #     num_outputs = len(self.encoder.bins)
        ...
        #         lambda obj: obj.set_value(subject_encoder.encode(self.slider.tracker.get_value()), anim=False)
        # )
...
class EncoderCollapse(Scene):
    ...
    def add_brick(self, bin_lower, bin_upper):
        ...
        # print(self.curr_id, sec_lower, sec_upper)
    ...
    def split_brick(self, cell_i):
        ...
        # whole_brick.suspend_updating()
        # whole_brick.resume_updating()
        ...
        # whole_fade_anim3 = FadeOut(whole_brick, run_time=0.0)
        # diff_anims.append(Succession(whole_brick_anim, whole_fade_anim3))
...
class AnimateEncoding(EncoderCollapse):
    ...
    def construct(self):
        ...
        self.play(Wait(1.0))
        self.play(self.tracker.animate.set_value(-0.001), run_time=5.0,
                  rate_func=rate_functions.linear)
        ...
        if len(all_anims) > 0:
            self.play(*all_anims)
            # self.play(*all_anims, run_time=0.5)

        self.play(Wait(1.0))

        for cell_i in range(self.curr_id):
            brick_vg = self.whole_blocks[cell_i]
            # self.remove(brick_vg)
            # brick_vg.resume_updating()
...
class AnimateTaperingWeightEncoder(EncoderCollapse):
...
class AnimateFixedWeightEncoder(EncoderCollapse):
    ...
    def construct(self):
        ...
        subject_encoder = MultiEncoder()
        for k in [7, 11, 13]:
            subject_encoder.add_encoder(FixedWeightEncoder(n=k, w=3))
...
class AnimateRandomizedPlaceCellEncoder(EncoderCollapse):
...
class AnimateManualCells(EncoderCollapse):
    ...
    def construct(self):
        ...
        # create and show brick objects manually chosen
