...
from shapely.ops import linemerge, polygonize_full, polygonize, unary_union
...
def draw_gnomes(ax, X_gnomes, num_bins, num_grids=1, fontsize=16, colors=None, annot=None):
    ...
    # ic(X_gnomes.shape, num_bins, num_grids)

    if annot is None:
        # annot = np.array("",shape=X_gnomes.shape, dtype=str)
        # print(X_gnomes.shape)
        annot = np.full(X_gnomes.shape, "", dtype=StringDType())
        for grid_i in range(num_grids):
            for i in range(num_bins):
                for j in range(num_bins):
                    annot[grid_i*num_bins + i, j] = "%d,%d" % (i, j)
                    # str_value = str(i) + ", " + str(j)
                    # ic(annot[i, j])
                    # annot[i, j] = str(str_value)
                    # ic(annot[i, j])
                    # ic(i, j, str_value, annot[i, j])


        # print(X_gnomes.shape, annot)
...
def draw_lines(ax, hgt, ortho_grid_vecs, ortho_grid_angles, points, artists=None, colors=None):
    ...
    for iter_index, pnt in enumerate(points):
        ...
        for dim_i in range(hgt.num_subspace_dims):
            ...
            point_to_origin_dist = compute_distance_to_plot_boundary(ortho_vec, bbox=((3.0, 3.0), (-3.0, -3.0)))
...
def build_hgt_artists(ax=None, hgt=None, colors=None):
    ...
    # grid artists, artists[grid_i][subspace_i] = np.empty([hgt.num_bins, hgt_bins], dtype=object)
    # elements of numpy array contain artist tuples of (rect, internal_text)
    artists = np.empty([hgt.num_grids, hgt.num_subspace_dims, hgt.num_bins, hgt.num_bins], dtype=object)
    ...
    for grid_i in range(num_grids):
        ...
        if num_subspace_dims == 1:
            ...
            displacements = np.apply_along_axis(compute_distance_to_plot_boundary, 1,
                                                ortho_vecs) + padding + (grid_i % 2) * 1.0
...
def create_text_rect_artists(ax, box_x, box_y, box_width, box_height, angle=0.0, linewidth=1.5, edgecolor='k', fontsize=8,
